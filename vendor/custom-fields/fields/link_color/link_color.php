 ...
 *
 */
if ( ! class_exists( 'SGU_FW_Field_link_color' ) ) {
  class SGU_FW_Field_link_color extends SGU_FW_Fields {
    ...
    public function render() {
      ...
      $color_props = array(
        'color'    => esc_html__( 'Normal', 'sgu_fw' ),
        'hover'    => esc_html__( 'Hover', 'sgu_fw' ),
        'active'   => esc_html__( 'Active', 'sgu_fw' ),
        'visited'  => esc_html__( 'Visited', 'sgu_fw' ),
        'focus'    => esc_html__( 'Focus', 'sgu_fw' )
      );
      ...
      foreach ( $color_props as $color_prop_key => $color_prop_value ) {
        ...
        if ( ! empty( $args[$color_prop_key] ) ) {
          ...
          echo '<div class="sgu_fw--left sgu_fw-field-color">';
          echo '<div class="sgu_fw--title">'. esc_attr( $color_prop_value ) .'</div>';
          echo '<input type="text" name="'. esc_attr( $this->field_name( '['. $color_prop_key .']' ) ) .'" value="'. esc_attr( $value[$color_prop_key] ) .'" class="sgu_fw-color"'. $default_attr . $this->field_attributes() .'/>';
          echo '</div>';
